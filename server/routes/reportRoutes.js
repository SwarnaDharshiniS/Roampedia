  ...
  try {
    ...
    if (visited.length > 0) {
      ...
      visited.forEach((country, index) => {
        ...
        doc.text(
          `${index + 1}. ${country.countryName} (${country.region}) - Visited: ${new Date(
            country.dateVisited
          ).toLocaleDateString()}`
        );
      });
...
router.get("/experience-journal", auth, async (req, res) => {
  try {
    ...
    doc.fontSize(24).font("Helvetica-Bold").text("Travel Experience Journal", {
      align: "center",
    });
    doc.moveDown();
    doc.fontSize(12).font("Helvetica").text(`${user.firstName} ${user.lastName}`, {
      align: "center",
    });
    doc.text(`Date: ${new Date().toLocaleDateString()}`, { align: "center" });
    doc.moveDown(2);
    ...
    experiences.forEach((exp, index) => {
      ...
      doc.text(
        `Duration: ${new Date(exp.fromDate).toLocaleDateString()} to ${new Date(
          exp.toDate
        ).toLocaleDateString()}`
      );
      doc.moveDown();

      doc.fontSize(11).font("Helvetica").text(exp.experience, { align: "justify" });
    });
...
router.get("/statistics", auth, async (req, res) => {
  try {
    ...
    doc.fontSize(24).font("Helvetica-Bold").text("Travel Statistics Report", {
      align: "center",
    });
    doc.moveDown(2);
...
router.get("/admin-reports/users-summary", auth, isAdmin, async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=users-summary.pdf"
    );
    doc.pipe(res);

    doc.fontSize(24).font("Helvetica-Bold").text("All Users Summary Report", {
      align: "center",
    });
    doc.moveDown();
    doc
    ...
    for (const user of users) {
      ...
      doc.text(
        `  Visited: ${visited} | Wishlist: ${wishlist} | Experiences: ${experiences}`
      );
      doc.moveDown(0.5);
    }
...
// =========================
router.get(
  "/admin-reports/country-popularity",
  auth,
  isAdmin,
  async (req, res) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        "attachment; filename=country-popularity.pdf"
      );
      doc.pipe(res);
      ...
      doc
        .fontSize(24)
        .font("Helvetica-Bold")
        .text("Country Popularity Report", { align: "center" });
      doc.moveDown(2);
...
router.get("/admin-reports/engagement", auth, isAdmin, async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=engagement-report.pdf"
    );
    ...
    doc
      .fontSize(24)
      .font("Helvetica-Bold")
      .text("User Engagement Report", { align: "center" });
    doc.moveDown(2);
    ...
    engagement.forEach((user, index) => {
      ...
      doc.text(
        `   Total Activity: ${user.total} (V: ${user.visited}, W: ${user.wishlist}, E: ${user.experiences})`
      );
      doc.moveDown(0.3);
    });
...
// =========================
router.get(
  "/admin-reports/system-statistics",
  auth,
  isAdmin,
  async (req, res) => {
    try {
      const [totalUsers, totalVisited, totalWishlist, totalExperiences] =
        await Promise.all([
      ...
      res.setHeader(
        "Content-Disposition",
        "attachment; filename=system-statistics.pdf"
      );
      ...
      doc.text(
        `Average Visited per User: ${(totalVisited / totalUsers).toFixed(2)}`
      );
      doc.text(
        `Average Experiences per User: ${(totalExperiences / totalUsers).toFixed(2)}`
      );
